...
const Game = () => {
  ...
  const startGame = () => {
    ...
    setGameOver(false);
    setActiveTab('FRUIT');
    setMatchedImage(getRandomImage('FRUIT').imageUrl);
  };
  ...
  const getRandomImage = category => {
    const categoryImages = imagesList.filter(image => image.category === category);
    return categoryImages[Math.floor(Math.random() * categoryImages.length)];
  };

  const handleTabClick = tabId => {
    setActiveTab(tabId);
    setMatchedImage(getRandomImage(tabId).imageUrl);
  };

  const handleThumbnailClick = thumbnailUrl => {
    if (thumbnailUrl === matchedImage) {
      setScore(prevScore => prevScore + 1);
      setMatchedImage(getRandomImage(activeTab).imageUrl);
    } else {
      endGame();
    }
  };
    ...
    <div> 
      ...
      {!gameOver ? (
        <div>
          <h2>Score: {score}</h2>
          <h2>Time: {time}</h2>
          <img src={matchedImage} alt="Matched Image" />
          <div>
            {tabsList.map(tab => (
              <button
                key={tab.tabId}
                onClick={() => handleTabClick(tab.tabId)}
                disabled={activeTab === tab.tabId}
              >
                {tab.displayText}
              </button>
            ))}
            ...
            {imagesList
              ...
              .map(image => (
                <img
                  ...
                  alt="Thumbnail"
                  onClick={() => handleThumbnailClick(image.thumbnailUrl)}
                />
              ))}
          </div>
        </div>
